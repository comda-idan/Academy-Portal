...
/**
 * AES-GCM encryption keyed from the admin password (PBKDF2).
 * Used to protect the GitHub PAT at rest in localStorage.
 *
 * Envelope format (JSON string):
 *   { v: 1, salt, iv, ct }    (all base64)
 *
 * Decryption requires the same password that was used to encrypt.
 * A wrong password throws (AES-GCM auth tag fails), so we can detect it cleanly.
 */
const TokenCrypto = {
  ITERATIONS: 150000,
  ...
  async encrypt(plaintext, password) {
    ...
    const key  = await this._deriveAesKey(password, salt);
    const enc  = new TextEncoder();
...
window.TokenCrypto = TokenCrypto;

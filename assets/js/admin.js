 ...
 *    The PAT is AES-GCM encrypted with a key derived from the admin password.
...
(() => {
  ...
  // localStorage layout:
  //   academy.adminCfg      - plaintext metadata only (owner, repo, branch)
  //   academy.adminTokenEnc - AES-GCM envelope of the PAT, keyed from password
  const CFG_KEY      = 'academy.adminCfg';
  const TOKEN_KEY    = 'academy.adminTokenEnc';
  ...
  let sessionPassword = null; // kept only in memory during an active session
  ...
  async function boot() {
    ...
    // Nothing is prefilled on the login form. GitHub config is only
    // touched after a successful password verification.
  }
  ...
  function wireUi() {
    ...
    // Post-login GitHub setup modal
    $('#gs-save').addEventListener('click', onGithubSetupSave);
    $('#gs-logout').addEventListener('click', () => {
      $('#github-setup-modal').classList.remove('open');
      onLogout();
    });

    // Settings tab: GitHub connection update
    $('#btn-save-github').addEventListener('click', onSettingsSaveGithub);
  }

  // ---------- Login ----------
  /**
   * Login flow:
   *   1. Verify username + password against auth.json (public file, PBKDF2 hash).
   *   2. Try to decrypt a previously-saved PAT with this password (AES-GCM).
   *      - On success: construct api, enter admin.
   *      - On failure (no token, or token saved under a different password): open setup modal.
   *
   * The GitHub credentials are NEVER present in the DOM before step 1 passes.
   */
  async function onLogin() {
    ...
    if (!auth || !auth.admin) return setLoginError('הגדרות האימות לא נטענו');
    ...
    // Try to use a previously-saved encrypted token.
    const cfg = loadCfg();
    const envelope = localStorage.getItem(TOKEN_KEY);
    if (cfg && cfg.owner && cfg.repo && envelope) {
      setLoginBusy(true);
      try {
        const token = await TokenCrypto.decrypt(envelope, password);
        const tentative = new GithubApi({ ...cfg, token });
        ...
        console.warn('stored token unusable, asking for a new one:', e.message);
        setLoginBusy(false);
      }
    }

    // No usable saved token -> prompt for GitHub credentials.
    openGithubSetup(cfg || autoGuessCfg());
  }
  ...
  function autoGuessCfg() {
    const auto = (typeof detectRepoFromUrl === 'function') ? detectRepoFromUrl() : null;
    return auto ? { owner: auto.owner, repo: auto.repo, branch: 'main' } : null;
  }

  // ---------- GitHub setup modal (first-time or after a failed decrypt) ----------
  function openGithubSetup(prefill) {
    $('#gs-owner').value  = prefill?.owner  || '';
    $('#gs-repo').value   = prefill?.repo   || '';
    $('#gs-branch').value = prefill?.branch || 'main';
    $('#gs-token').value  = '';
    $('#gs-status').textContent = '';
    $('#github-setup-modal').classList.add('open');
  }

  async function onGithubSetupSave() {
    const owner  = $('#gs-owner').value.trim();
    const repo   = $('#gs-repo').value.trim();
    const branch = $('#gs-branch').value.trim() || 'main';
    const token  = $('#gs-token').value.trim();
    const statusEl = $('#gs-status');

    if (!owner || !repo || !token) {
      statusEl.className = 'status error';
      statusEl.textContent = 'חסרים owner / repo / token';
      return;
    }
    if (!sessionPassword) {
      statusEl.className = 'status error';
      statusEl.textContent = 'סשן לא פעיל — אנא התחבר מחדש.';
      return;
    }

    $('#gs-save').disabled = true;
    try {
      const tentative = new GithubApi({ owner, repo, branch, token });
      await tentative.whoami();
      saveCfg({ owner, repo, branch });
      const envelope = await TokenCrypto.encrypt(token, sessionPassword);
      localStorage.setItem(TOKEN_KEY, envelope);
      api = tentative;
      $('#gs-token').value = '';
      $('#github-setup-modal').classList.remove('open');
      await enterAdmin();
      ...
      statusEl.className = 'status error';
      statusEl.textContent = `חיבור ל-GitHub נכשל: ${e.message}`;
    } finally {
      $('#gs-save').disabled = false;
    }
  }

  // ---------- Settings: update GitHub connection from inside the admin ----------
  async function onSettingsSaveGithub() {
    const owner  = $('#gh-owner').value.trim();
    const repo   = $('#gh-repo').value.trim();
    const branch = $('#gh-branch').value.trim() || 'main';
    const typedToken = $('#gh-token').value.trim();
    const statusEl = $('#gh-status');

    if (!owner || !repo) {
      statusEl.className = 'status error';
      statusEl.textContent = 'owner + repo חובה';
      return;
    }
    if (!sessionPassword) {
      statusEl.className = 'status error';
      statusEl.textContent = 'סשן לא פעיל.';
      return;
    }

    $('#btn-save-github').disabled = true;
    try {
      let token = typedToken;
      if (!token) {
        const envelope = localStorage.getItem(TOKEN_KEY);
        if (!envelope) throw new Error('אין טוקן שמור — הזן טוקן חדש');
        token = await TokenCrypto.decrypt(envelope, sessionPassword);
      }
      const tentative = new GithubApi({ owner, repo, branch, token });
      await tentative.whoami();
      saveCfg({ owner, repo, branch });
      const newEnvelope = await TokenCrypto.encrypt(token, sessionPassword);
      localStorage.setItem(TOKEN_KEY, newEnvelope);
      api = tentative;
      $('#gh-token').value = '';
      statusEl.className = 'status success';
      statusEl.textContent = 'חיבור GitHub עודכן.';
    } catch (e) {
      statusEl.className = 'status error';
      statusEl.textContent = `עדכון נכשל: ${e.message}`;
    } finally {
      $('#btn-save-github').disabled = false;
    }
  }

  function fillSettingsGithubForm() {
    const cfg = loadCfg();
    $('#gh-owner').value  = cfg?.owner  || '';
    $('#gh-repo').value   = cfg?.repo   || '';
    $('#gh-branch').value = cfg?.branch || 'main';
    $('#gh-token').value  = '';
    $('#gh-token').placeholder = localStorage.getItem(TOKEN_KEY)
      ? 'השאר ריק כדי לא להחליף'
      : 'github_pat_...';
  ...
  function onLogout() {
    ...
    api = null;
    // The encrypted token stays in localStorage - it's useless without the password.
  ...
  function activateTab(id) {
    ...
    if (id === 'tab-settings')   fillSettingsGithubForm();
  }
  ...
  async function onChangePassword() {
    ...
    try {
      ...
      // Re-encrypt the stored PAT under the new password so the next login still works.
      const envelope = localStorage.getItem(TOKEN_KEY);
      if (envelope) {
        try {
          const token = await TokenCrypto.decrypt(envelope, sessionPassword);
          const reEnv = await TokenCrypto.encrypt(token, next);
          localStorage.setItem(TOKEN_KEY, reEnv);
        } catch (err) {
          localStorage.removeItem(TOKEN_KEY);
          console.warn('PAT re-encryption failed, dropping stored token:', err);
        }
      }
